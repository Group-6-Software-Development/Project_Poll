...
class TestReviewRoutes(unittest.TestCase):
    ...
    def test_create_review_route_fail(self):
        register_response = self.app.post('/api/user/register', json={
            'firstName': 'Test',
            'lastName': 'Review_Create',
            'email': 'test@review_create_fail.fi',
            'password': 'asd123!'
        })
        decoded_token = register_response.data.decode('utf-8')
        decoded_token = json.loads(decoded_token)['token']
    ...
    def test_create_review_route_bad_request(self):
        register_response = self.app.post('/api/user/register', json={
            'firstName': 'Test',
            'lastName': 'Review_Create',
            'email': 'test@review_create_bad_request.fi',
            'password': 'asd123!'
        })
        decoded_token = register_response.data.decode('utf-8')
        decoded_token = json.loads(decoded_token)['token']
    ...
    def test_get_reviews_bad_request(self):
        register_response = self.app.post('/api/user/register', json={
            'firstName': 'Test',
            'lastName': 'Reviews_Get',
            'email': 'test@reviews_get_bad_request.fi',
            'password': 'asd123!'
        })
        ...
        response = self.app.get(f'/api/review/reviews/',
                                headers={'Authorization': f'Bearer {decoded_token}'})
